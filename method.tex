...
该优化过程使用PyTorch\citep{pytorch}和nvdiffrast实现，被优化的参数是表示SE(3)变换的6个参数。
优化的目标函数为公式\eqref{eq:loss_n_tilde}，且未使用任何正则项。
本文使用Adam\citep{adam}优化器，学习率为$10^{-3}$，其余参数为默认值。
...
可见若使用不准确的的黑色或数学期望背景模型，该优化问题则很难收敛到期望的结果。
(c)、(d)、(e)对比可以看出越准确的背景模型可产生越好的优化结果，这证实了在一般
