...
因此，对误差的控制贯穿于采集流程的各个环节，指导整个采集平台的软硬件设计。
...
% \begin{algorithm}
%     \caption{闪光灯触发延迟快速标定}
%     \label{alg:flash_delay}
%     \begin{algorithmic}[1]
%         \State 将相机快门速度设置到1/2秒
%         \State $o \gets 0$
%         \Repeat
%             \State 将闪光灯触发时间设置到相机后$o$毫秒，拍摄一张照片$\mathcal{I}_o$
%             \State $o \gets o + 100$
%         \Until{$\mathcal{I}_o$曝光全亮}
%         \State $o_{\mathrm{upper}}\gets o, o \gets 0$
%         \While{$\mathcal{I}_o$曝光全亮}
%             \State 将闪光灯触发时间设置到相机后$o$毫秒，拍摄一张照片$\mathcal{I}_o$
%             \State $o \gets o - 100$
%         \EndWhile
%         \State $o_{\mathrm{lower}}\gets o$
%         \While{true}
%             \State $o \gets (o_\mathrm{upper} + o_\mathrm{lower}) / 2$
%             \State 将闪光灯触发时间设置到相机后$o$毫秒，拍摄一张照片$\mathcal{I}_o$
%             \If{$\mathcal{I}_o$曝光全亮}
%                 \State $o_\mathrm{upper} \gets o$
%             \ElsIf{$\mathcal{I}_o$曝光全暗}
%                 \State $o_\mathrm{lower} \gets o$
%             \Else
%                 \State \textbf{break}
%             \EndIf
...
本文首先使用COLMAP\citep{colmap}完成人脸的稀疏以及稠密点云重建。
由于COLMAP原应用场景中视角相对密集且数量多，但本方案的视角数量受限于相机数量，较为稀疏，因此需要调节COLMAP的特征点匹配参数，减少不同视角间需要的匹配数量，以提高成功率。
然后，本文使用FlameFitting\footnote{https://github.com/Rubikplayer/flame-fitting}工具，将FLAME 3D模型配准到稠密点云上，得到初步的人脸几何形状。
然而，该配准的效果并不理想。因此本文使用Blender同时导入点云和几何形状，并手动调整模型顶点位置以完成更精确的配准。
最后，本文通过第\ref{sec:method_photo2tex}节介绍的方法，将照片映射到UV空间，每个视角获得一张纹理贴图$\mathcal{T}_i$，$i$为视角的索引，再将这些纹理贴图通过类似公式\eqref{eq:texture_fusion}的方式进行融合，最终获得一个带纹理的3D人脸模型。

其中纹理融合方式总体为每张纹理图的加权平均，每张图中的每个像素的权重定义为：
\begin{equation}
\label{eq:colmap_texture_fusion_weight}
w_{i,j} = \begin{cases}
    \frac{\mathbf{v}\cdot\mathbf{n}}{\|\mathbf{v}\|} & \text{if像素位于有效区域} \\
    0 & \text{otherwise}
\end{cases}
\text{，}
\end{equation}
其中$\mathbf{v}$、$\mathbf{n}$分别为顶点3D坐标和单位法向，$j$为像素索引。
最终融合的纹理图为：
\begin{equation}
\label{eq:colmap_texture_fusion}
\mathcal{T}_j = \frac{\sum_i w_{i,j}\mathcal{T}_i}{\sum_i w_{i,j}}
\text{。}
\end{equation}
图\ref{fig:colmap_result}展示了该方法的结果。
从该结果中看，虽然COLMAP输出的点云精度较高，但受限于配准的效果，不同视角的纹理尚不能达到像素级的对齐，因此在多张纹理融合后反而变得模糊了。
此外，本初步验证并未试图对纹理进行符合物理的建模。
本方法仅通过多视角纹理平均来减弱和视角有关的镜面反射效果，从而让该模型能一定程度上适应新的视角和光照，其效果是非常有限的。

\begin{figure}
    \centering
    \TODO{结果图}
    \caption{利用COLMAP的重建结果}
    \label{fig:colmap_result}
\end{figure}
