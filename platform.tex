...
令$\overrightarrow{\mathbf{O}\mathbf{D}}$与球面相切，可由$\overrightarrow{\mathbf{O}\mathbf{D}} \perp \overrightarrow{\mathbf{C}\mathbf{D}}$列方程求解$\phi$：
\begin{equation}
\begin{aligned}
...
本节将介绍上述方案的具体实现细节，并对该方案所采集到的数据开展了一些验证性的实验。
...
控制器无需供电，它采用机械按钮的形式完成控制线和地线的短接，从而触发相机快门。
...
\begin{figure}
    ...
    \end{minipage}%
\end{figure}
在标定完成后，本文本文将最终相机参数导出为glTF格式，以便在Blender等专业3D建模软件中进行可视化。
如图\ref{fig:calib_vis}是本实例的标定结果在Blender中的可视化界面。

然而，可视化只能提供一个直观的感受。本节将对标定的精度进行量化分析，并列举可能影响标定精度的因素。
