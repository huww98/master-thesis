...
其中$\mathbf{X}$为相机坐标系下的点的三维坐标，$(\mathbf{x}_x/\mathbf{x}_z,\mathbf{x}_y/\mathbf{x}_z)$即是该点投影到图像上的二维坐标。
...
\begin{equation}
    \mathbf{P}_\mathrm{cam} = \begin{bmatrix}
        f\mathbf{\rho}_x & 0 & \frac{\mathbf{r}_x}{2} \\
        0 & f\mathbf{\rho}_y & \frac{\mathbf{r}_y}{2} \\
        0 & 0 & 1
    \end{bmatrix}
    \text{。}
\end{equation}
进一步地，记裁剪框的左下角和右上角坐标分别为$\mathbf{p}_\mathrm{min},\mathbf{p}_\mathrm{max}\in\mathbb{R}^2$，
则裁剪框的投影矩阵为：
\begin{equation}
    \mathbf{P}_\mathrm{crop} = \begin{bmatrix}
        \frac{\mathbf{r}_x}{\mathbf{p}_\mathrm{max,x}-\mathbf{p}_\mathrm{min,x}} & 0 & -\frac{\mathbf{r}_x\mathbf{p}_\mathrm{min,x}}{\mathbf{p}_\mathrm{max,x}-\mathbf{p}_\mathrm{min,x}} \\
        0 & \frac{\mathbf{r}_y}{\mathbf{p}_\mathrm{max,y}-\mathbf{p}_\mathrm{min,y}} & -\frac{\mathbf{r}_y\mathbf{p}_\mathrm{min,y}}{\mathbf{p}_\mathrm{max,y}-\mathbf{p}_\mathrm{min,y}} \\
        0 & 0 & 1
    \end{bmatrix}
    \text{。}
\end{equation}
