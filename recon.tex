...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \node (target) [image] {\includegraphics[height=1.55in]{figures/diffrast_face/04114/target_hs}};

        \node (my) [image, right=of target] {\includegraphics[height=1.55in]{figures/diffrast_face/04114/final_rerendered}};

        \node (no_sdf) [image, right=of my] {\includegraphics[height=1.55in]{figures/diffrast_face/04114/no_sdf}};

        \small
        \node [below=of target] {(a)目标图像};
        \node [below=of my] {(b)本文方法};
        \node [below=of no_sdf] {(c)不特殊处理};
    \end{tikzpicture}
    \caption{基于SDF的裁剪边缘处理方法效果}
    \label{fig:edge_gradient}
\end{figure}
本文提出的可微分渲染方法应用在3D人脸重建时，由于人脸模型的人工裁剪导致出现在图片中不存在的边缘。
...
例如图\ref{fig:edge_gradient}c中，算法将本应位于下巴处的网格用于拟合了照片中的脖子区域。
使用本文提出的方法即可过滤掉这部分应用在裁剪边缘的梯度，从而避免这种问题。
