...
与之前方法\citep{deep3d,GuoZCJZ19}保持一致，本文使用了BFM 2009模型\citep{BFM}中的身份和纹理基，以及从Face Warehouse\citep{FaceWarehouse}中创建的表情基。
...
\paragraph{光栅化}
该步骤直接使用了nvdiffrast中的光栅化算法，
其输入为顶点的裁切空间坐标$\mathbf{S}_\mathrm{clip}$和顶点索引，
输出为每个像素的三角形索引和其在该三角形中的重心坐标，
以及重心坐标对图像像素坐标的梯度。
